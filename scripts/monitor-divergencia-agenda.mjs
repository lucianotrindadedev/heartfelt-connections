 ...
 *
 *  2) SLOT PRESO — selected_slot_iso ∉ offered_slots E sem appointment_id. Seleção
 *     "presa" numa oferta anterior → booking falha técnico e trava/escala (caso
 *     Costa Lima Recreio, Neymar Junior).
 *
 * Espelha a lógica dos guards do scheduler (affirmedDatesFromAssistant +
 * ddmmInBrt + selectedSlotIsStale em booking-template.ts).
 *
 * Uso:
...
// resolve contas que ainda não estão no mapa (varredura de população diferente)
const staleAgentIds = [
...
const missingAcc = [
  ...new Set(
    Object.values(agentToAccount).filter((aid) => aid && !(aid in accountName)),
  ),
];
...
if (staleItens.length) {
  const porConta = {};
  for (const s of staleItens) porConta[s.conta] = (porConta[s.conta] ?? 0) + 1;
  console.log("\n── por conta ──");
  for (const [k, v] of Object.entries(porConta).sort((a, b) => b[1] - a[1]))
    console.log(`  ${String(v).padStart(3)}  ${k}`);
...
if (csvArg) {
  ...
  const lines = [
    ...
    ...buckets.DIVERGENTE.map((d) =>
      ["DIVERGENTE", d.conta, d.fone, d.agendado, `afirmado: ${d.afirmado}`, d.conv, d.atualizado, d.evidencia]
        .map(esc)
        .join(","),
    ),
    ...staleItens.map((s) =>
