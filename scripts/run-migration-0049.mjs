/**
 * Aplica a migration 0049_clinup_professionals.sql (coluna boolean
 * clinup_config.professionals) via Supabase REST (execute_sql RPC)
...
const sql = readFileSync(resolve(root, "migrations/0049_clinup_professionals.sql"), "utf8");
const verifySql = `select column_name, data_type, column_default from information_schema.columns where table_name = 'clinup_config' and column_name = 'professionals'`;

console.log("🔄  Aplicando migration 0049_clinup_professionals…\n");
try {
  await execSql(sql);
