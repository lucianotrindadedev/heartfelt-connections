/**
 * Aplica a migration 0053_clinic_experts_unidades.sql (coluna jsonb unidades em
 * clinic_experts_config (Clinic Experts multi-unidade))
 * via Supabase REST (execute_sql RPC) com fallback para /pg/query.
 * Uso: node scripts/run-migration-0053.mjs
 */
...
const sql = readFileSync(resolve(root, "migrations/0053_clinic_experts_unidades.sql"), "utf8");
const verifySql = `select column_name, data_type from information_schema.columns where table_name = 'clinic_experts_config' and column_name = 'unidades'`;

console.log("🔄  Aplicando migration 0053_clinic_experts_unidades…\n");
try {
  await execSql(sql);
