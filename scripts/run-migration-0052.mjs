/**
 * Aplica a migration 0052_google_sheets.sql (tabela google_sheets_config —
 * tokens OAuth de planilha + lista de planilhas consultáveis pelo agente)
 * via Supabase REST (execute_sql RPC) com fallback para /pg/query.
 * Uso: node scripts/run-migration-0052.mjs
 */
...
const sql = readFileSync(resolve(root, "migrations/0052_google_sheets.sql"), "utf8");
const verifySql = `select column_name, data_type from information_schema.columns where table_name = 'google_sheets_config' order by ordinal_position`;

console.log("🔄  Aplicando migration 0052_google_sheets…\n");
try {
  await execSql(sql);
