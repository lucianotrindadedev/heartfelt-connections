/**
 * Aplica a migration 0046_model_temperatures.sql (coluna jsonb
 * account_llm_config.model_temperatures) via Supabase REST (execute_sql RPC)
...
const sql = readFileSync(resolve(root, "migrations/0046_model_temperatures.sql"), "utf8");
const verifySql = `select column_name, data_type, column_default from information_schema.columns where table_name = 'account_llm_config' and column_name = 'model_temperatures'`;

console.log("🔄  Aplicando migration 0046_model_temperatures…\n");
try {
  await execSql(sql);
