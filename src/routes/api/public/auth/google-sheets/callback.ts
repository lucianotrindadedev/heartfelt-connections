// GET /api/public/auth/google-sheets/callback
// Recebe o code do consentimento de PLANILHA (escopo spreadsheets.readonly),
// troca por tokens e salva criptografados em google_sheets_config.
//
// Callback separado do /auth/google/callback de propósito: o consentimento do
// Calendar e o da planilha são independentes, e cada um grava na sua tabela —
// conectar planilha nunca sobrescreve (nem invalida) o token da agenda.
...
export const Route = createFileRoute("/api/public/auth/google-sheets/callback")({
  server: {
    handlers: {
      GET: async ({ request }) => {
        ...
        try {
          const tokenRes = await fetch(TOKEN_URL, {
            ...
            body: new URLSearchParams({
              ...
              redirect_uri:
                ...
                `${url.origin}/api/public/auth/google-sheets/callback`,
          ...
          const info = infoRes.ok ? ((await infoRes.json()) as { email?: string }) : null;

          const expiresAt = new Date(Date.now() + (tokens.expires_in ?? 3600) * 1000).toISOString();

          const sb = getSelfhost();
          // Sem `planilhas` no upsert: reconectar não pode apagar a lista já
          // configurada (o default '[]' só vale na inserção).
          await sb.from("google_sheets_config").upsert(
            {
              account_id: state,
...
function htmlResponse(success: boolean, errorMsg: string | null, email?: string | null) {
  const html = success
    ? `<!DOCTYPE html>
<html lang="pt-BR">
<head><meta charset="UTF-8"><title>Google Sheets Conectado</title>
...
<p>Agora cadastre as planilhas que o agente pode consultar.</p>
